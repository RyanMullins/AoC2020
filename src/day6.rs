...
fn count_affirmative_deduplicated (group_answers: Vec<&str>) -> u32 {
    let mut group_uniqs: Vec<u32> = Vec::new();

    for answers in &group_answers {
        let uniq_chars: HashSet<char> = answers.chars().filter(|c| c.is_alphabetic()).collect();
        group_uniqs.push(uniq_chars.len() as u32);
    }

    group_uniqs.iter().fold(0, |total, count| total + count)
}

fn count_unanimous_deduplicated (group_answers: Vec<&str>) -> u32 {
    let mut group_unanimous: Vec<u32> = Vec::new();

    for all_answers in &group_answers {
        let mut uniq_chars: HashMap<char, usize> = HashMap::new();
        let individual_answers: Vec<&str> = all_answers.split("\n").collect();
        let group_size: usize = individual_answers.len();

        for answers in  &individual_answers {
            let yeses: Vec<char> = answers.chars().filter(|c| c.is_alphabetic()).collect();

            for yes in &yeses {
                if uniq_chars.contains_key(yes) {
                    uniq_chars.insert(*yes, uniq_chars.get(yes).unwrap() + 1);
                } else {
                    uniq_chars.insert(*yes, 1);
                }
            }
        }

        group_unanimous.push(uniq_chars.iter()
            .filter(|&(_key, value)| *value == group_size)
            .collect::<HashMap<&char, &usize>>()
            .keys().len() as u32);
    }

    group_unanimous.iter().fold(0, |total, count| total + count)
}

pub fn solve_d6 () {
    let group_answers: Vec<&str> = "f
    v
    ki
    ...
    println!("Day 6 Problem 1: {}", count_affirmative_deduplicated(group_answers.to_vec()));
    println!("Day 6 Problem 2: {}", count_unanimous_deduplicated(group_answers.to_vec()));
}
