...
fn count_rule_follwers (candidates:&str) -> u32{
    lazy_static! {
        static ref RULE_PASS_REGEX: Regex = Regex::new(r"(?m)^(?P<low>\d+)\-(?P<high>\d+) (?P<lookfor>\D): (?P<password>\D+)$").unwrap();
    }
    
    let mut followers: u32 = 0;

    for candidate in RULE_PASS_REGEX.captures_iter(&candidates) {
        let low: usize = candidate.name("low").unwrap().as_str().parse::<usize>().unwrap();
        let high: usize = candidate.name("high").unwrap().as_str().parse::<usize>().unwrap();
        let lookfor = candidate.name("lookfor").unwrap().as_str().chars().next().unwrap();
        let password = candidate.name("password").unwrap().as_str();
        let count: usize = password.chars().filter(|pchar| pchar.eq(&lookfor)).collect::<Vec<char>>().len();
...
fn count_rule_follwers_again (candidates:&str) -> u32{
    lazy_static! {
        static ref RULE_PASS_REGEX: Regex = Regex::new(r"(?m)^(?P<low>\d+)\-(?P<high>\d+) (?P<lookfor>\D): (?P<password>\D+)$").unwrap();
    }
    ...
    for candidate in RULE_PASS_REGEX.captures_iter(&candidates) {
        let low: usize = candidate.name("low").unwrap().as_str().parse::<usize>().unwrap() - 1;
        let high: usize = candidate.name("high").unwrap().as_str().parse::<usize>().unwrap() - 1;
        let lookfor = candidate.name("lookfor").unwrap().as_str().chars().next().unwrap();
        let password = candidate.name("password").unwrap().as_str().chars().collect::<Vec<char>>();
        if  (password[low] == lookfor && password[high] != lookfor) || 
            (password[low] != lookfor && password[high] == lookfor) {
            followers = followers + 1;
        } 
    }
...
pub fn solve_d2 () {
    let rules_and_passwords = "3-6 s: ssdsssss
...
9-10 f: fffpffffjff
1-5 d: ndtdc";
    let rule_followers: u32 = count_rule_follwers(&rules_and_passwords);
    println!("Day 2 Problem 1: {}", rule_followers);
    let position_followers: u32 = count_rule_follwers_again(&rules_and_passwords);
    println!("Day 2 Problem 2: {}", position_followers);
}
