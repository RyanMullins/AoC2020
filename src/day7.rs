...
fn derive_rules (rule_descriptions: Vec<&str>) -> HashMap<&str, HashMap<&str, usize>> {
    lazy_static! {
        static ref BAG_ALLOWS_REGEX: Regex = Regex::new(r"^(?P<bag>[ \w]+)s{0,1} contain (?P<allows>[\d\w ,]+)").unwrap();
        static ref ALLOWED_COUNT_REGEX: Regex = Regex::new(r"(?P<count>\d+) (?P<bag>[\w ]+)s{0,1}[\.,]{0,1}").unwrap();
    }

    let mut rules: HashMap<&str, HashMap<&str, usize>> = HashMap::new();

    for rule_desc in &rule_descriptions {
        for bag_allows in BAG_ALLOWS_REGEX.captures_iter(rule_desc) {
            let bag: &str = bag_allows.name("bag").unwrap().as_str();
            let bag_rules = rules.entry(bag).or_insert(HashMap::new());
            let allows: &str = bag_allows.name("allows").unwrap().as_str();

            for allowed_count in ALLOWED_COUNT_REGEX.captures_iter(allows) {
                let contained_bag: &str = allowed_count.name("bag").unwrap().as_str();
                let count_str: &str = allowed_count.name("count").unwrap().as_str();
                let count: usize = usize::from_str_radix(count_str, 10).unwrap();
                println!("INFO - day7::derive_rules - Bag {} allows {} {}", bag, count, contained_bag);
                bag_rules.insert(contained_bag, count);
            }
        }
    }
...
fn count_eventual_gold_bags (rule_defs: HashMap<&str, HashMap<&str, usize>>) -> usize {
    let mut eventual_containers: Vec<&str> = Vec::new();
    eventual_containers.push("shiny gold bag");
    let mut current_length: usize = 0;

    while current_length < eventual_containers.len() {
        current_length = eventual_containers.len();

        for (&_bag, &rules) in &rule_defs {
            for contains in rules.keys() {
    ...
    eventual_containers.len()
}

pub fn solve_d7 () {
    let rule_descriptions: Vec<&str> = "posh blue bags contain 5 plaid chartreuse bags, 3 plaid lime bags.
...
plaid green bags contain 4 dark brown bags, 1 shiny orange bag.".split("\n").collect();

    let rules = derive_rules(rule_descriptions);
    println!("Day 7 Problem 1: {}", count_eventual_gold_bags(rules));
}