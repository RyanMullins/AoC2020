...
fn validate_credentials (passports:Vec<&str>, extra_validation: bool) -> u32 {
    lazy_static! {
        static ref KEY_VALUE_REGEX: Regex = Regex::new(r"(?P<key>\w+):(?P<value>[#\w]+)").unwrap();
        static ref HEX_COLOR_REGEX: Regex = Regex::new(r"^#[0-9a-f]{6}$").unwrap();
    ...
    for passport in &passports {
        ...
        let passport_hashmap: HashMap<&str, &str> = KEY_VALUE_REGEX.captures_iter(passport)
            .map(|cap| (cap.name("key").unwrap().as_str(), cap.name("value").unwrap().as_str()))
            .collect();

        if passport_hashmap.keys().len() < 7 {
            passing = false;
        }

        if  passport_hashmap.keys().len() >= 7 && (
            passport_hashmap.contains_key("byr") == false ||
            passport_hashmap.contains_key("eyr") == false ||
            passport_hashmap.contains_key("iyr") == false ||
            passport_hashmap.contains_key("ecl") == false ||
            passport_hashmap.contains_key("hcl") == false ||
            passport_hashmap.contains_key("hgt") == false ||
            passport_hashmap.contains_key("pid") == false ) {
            passing = false;
        }

        if passing && extra_validation {
            for (k, v) in passport_hashmap.iter() {
                let key: &str = k as &str;
                let value: &str = v as &str;

                if  (key == "byr" && (value.len() != 4 || !("1920" <= value && value <= "2002"))) ||
                    (key == "eyr" && (value.len() != 4 || !("2020" <= value && value <= "2030"))) ||
                    (key == "iyr" && (value.len() != 4 || !("2010" <= value && value <= "2020"))) ||
                    (key == "ecl" && !vec!["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].contains(&value)) ||
                    (key == "hcl" && HEX_COLOR_REGEX.find(value) == None) ||
                    (key == "pid" && PID_REGEX.find(value) == None) ||
                    key == "hgt" && (
                        !(value.contains("cm") || value.contains("in")) ||
                        (value.contains("cm") && !("150cm" <= value && value <= "193cm")) ||
                        (value.contains("in") && !("58in" <= value && value <= "77in"))) {
                    passing = false;
                }
            }
        }
...
pub fn solve_d4 () {
    let passports: Vec<&str> = "eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
    ...
    hcl:#888785 cid:108 hgt:160cm".split("\n\n").collect();

    println!("Day 4 Problem 1: {}", validate_credentials(passports.to_vec(), false));
    println!("Day 4 Problem 2: {}", validate_credentials(passports.to_vec(), true));
}